...
namespace SLZ.Marrow
{
	public class HeptaRig : Rig
	{
		...
		protected SimpleTransform ExtrapolateThorLumbar(SimpleTransform inT1, Quaternion inSpineRot, Quaternion inPelvisRot, SLZ.VRMK.Avatar avatar, out SimpleTransform outSpine)
		{
			outSpine = default(SimpleTransform);
			return default(SimpleTransform);
		}
		...
		protected (float, float) SolveClavTwist(Vector3 axis, float handTwist, float armAngleRemap, float fwdDegrees, float backDegrees, float upDegrees, float downDegrees, bool isLeft)
		{
			return default((float, float));
		}
		...
		protected void ApplyProportions(Quaternion rootRotation, SimpleTransform head, ref SimpleTransform palmLf, ref SimpleTransform palmRt, SLZ.VRMK.Avatar avatar, BodyPose bodyPose)
		{
		}
		...
		protected void ExtractTorsoProximity(ref SimpleTransform palmLf, ref SimpleTransform palmRt, Rig inRig, SLZ.VRMK.Avatar avatar, Vector3 handLfBoundsPos, Vector3 handLfBoundsNeg, Vector3 handRtBoundsPos, Vector3 handRtBoundsNeg, ref BodyPose bodyPose)
		{
		}

		protected void ApplyTorsoProximity(ref SimpleTransform palmLf, ref SimpleTransform palmRt, SLZ.VRMK.Avatar avatar, BodyPose bodyPose, Vector3 handLfBoundsPos, Vector3 handLfBoundsNeg, Vector3 handRtBoundsPos, Vector3 handRtBoundsNeg, float twoHanded = 0f)
		{
		}
		...
		protected Vector3 HandTorsoSpacewarp(ref SimpleTransform palm, Vector3 pointBoundsPos, Vector3 pointBoundsNeg, SLZ.VRMK.Avatar avatar, SimpleTransform head, SimpleTransform sternum, SimpleTransform spine, SimpleTransform hipCenter, Quaternion handInSpine, float yPercRemapped, Vector2 sin, float t, Vector3 spineInPointNormed)
		{
			return default(Vector3);
		}
