        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUILayout.VerticalScope())
            {
                ...
                if (helpText)
                {
                    ...
                    EditorGUILayout.Space(padding);
                    defaultInspector = EditorGUILayout.Toggle("Show the Default Inspector", defaultInspector);
                    EditorGUILayout.Space(padding);
                }
        ...
        private void OnSceneGUI()
        {
            ...
            if (_hardTorsoHandles)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (activeHandleLabelName != null && activeHandleLabelName != "")
                    {
                        undoDetails += ": " + activeHandleLabelName;
                    }

                    if (bustChanged)
                        undoDetails += ": Bust Core";
                    if (waistChanged)
                        undoDetails += ": Waist Core";
                    if (hipChanged)
                        undoDetails += ": Hip Core";
                    if (crotchChanged)
                        undoDetails += ": Crotch Core";
                    Undo.RecordObject(target, "Modify Avatar Body" + undoDetails);
            ...
            if (_softTorsoHandles)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    if (breastDirty)
                        avatar.bulgeBreast.Copy(outBreast);
                    if (abdomenDirty)
                        avatar.bulgeAbdomen.Copy(outAbdomen);
                    if (groinDirty)
                        avatar.bulgeGroin.Copy(outGroin);
                }
            }
        }
